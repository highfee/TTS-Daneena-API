...
EMOTION_PROFILES = {
    'Angry':    {'tempo_factor': 1.10, 'loudness_db':  5.0},
    'Happy':    {'tempo_factor': 1.08, 'loudness_db':  3.0},
    'Neutral':  {'tempo_factor': 1.00, 'loudness_db':  0.0},
    'Sad':      {'tempo_factor': 0.82, 'loudness_db': -3.0},
    'Surprise': {'tempo_factor': 1.06, 'loudness_db':  4.0},
    'Fear':     {'tempo_factor': 1.12, 'loudness_db':  2.0},
    'Disgust':  {'tempo_factor': 0.90, 'loudness_db': -2.0},
    'Excited':  {'tempo_factor': 1.15, 'loudness_db':  4.0},
}
...
def postprocess(wav: np.ndarray, sr: int, emotion: str) -> np.ndarray:
    ...
    # Tempo only — least artifact-prone operation
    if abs(profile['tempo_factor'] - 1.0) > 0.01:
        wav = librosa.effects.time_stretch(wav, rate=profile['tempo_factor'])

    # Loudness
    if profile['loudness_db'] != 0.0:
        factor = 10 ** (profile['loudness_db'] / 20.0)
        wav = wav * factor
